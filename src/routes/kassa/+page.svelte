  ...
  async function onSearchInput() {
    ...
    debounceTimer = setTimeout(async () => {
      ...
      try {
        ...
        // Exact barcode or packetcode → add immediately, clear input
        const exact = results.find((p) => p.barcode === query || p.packetcode === query);
        if (exact) {
          addToCart(exact);
          searchInput = "";
          searchResults = [];
      ...
      <div class="flex items-center gap-2 border-2 border-ring rounded-lg px-3 py-2 bg-background shadow-sm focus-within:border-primary transition-colors">
        ...
        {#if searchInput}
          <button
            ...
            onclick={() => { searchInput = ""; searchResults = []; showDropdown = false; inputEl?.focus(); }}
          >✕</button>
        {/if}
      </div>
